 ...
 * 
 * Update 7/12/2015, Hugh Huynh
 * Changed functionality to allow toilets (denoted by the 'w' character)
 */
	...
	$.fn.seatCharts = function (setup) {
					...
					fn.node()
						...
						.on('keydown',    (function(seat, $seat) {
							...
							return function (e) {
								...
								switch (e.which) {
									case 9:
										// shift + tab will focus on the previous input element
										if(e.shiftKey){
											seat.blur();
											$(this).closest('.seatCharts-container').parent().prev().find("input:first").focus();
										}
										break;
		...
		$.each(settings.map, function(row, characters) {
			...
			$.each(characters.match(/[a-z_]{1}(\[[0-9a-z_]{0,}(,[0-9a-z_ ]+)?\])?/gi), function (column, characterParams) { 
				...
				$row.append((character != '_' && character != 'w') ?
					...
					})(settings.naming) : (character === 'w') ? 
						//this is just a toilet (w)
						$('<div>wc</div>').addClass('seatCharts-cell seatCharts-toilet')
		...
		settings.legend.items.length ? (function(legend) {
			...
			var $ul = $('<ul></ul>')
				.addClass('seatCharts-legendList row')
				.appendTo($container);
			
			$.each(legend.items, function(index, item) {
				$ul.append(
					$('<li></li>')
						.addClass('seatCharts-legendItem col-xs-6 col-sm-12')
						.append(
							$('<div><span class="legend-toilet">wc</span></div>')
