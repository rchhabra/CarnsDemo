...
<c:forEach var="entry" items="${passengerTypeQuantityList}" varStatus="i">
	<div class="col-xs-4 col-sm-4">
		<label for="y_${fn:escapeXml(entry.passengerType.code)}">
			<spring:theme var="passengerTypeLabel" code="text.cms.accommodationfinder.passenger.${entry.passengerType.code}"
				text="Passenger" />
			...
			<c:choose>
				<c:when test="${not empty passengerTypeMaxQuantityMap}">
					<c:forEach begin="0" end="${passengerTypeMaxQuantityMap[entry.passengerType.code]}" varStatus="loop">
						<form:option value="${loop.index}" htmlEscape="true"> ${fn:escapeXml(loop.index)} </form:option>
					</c:forEach>
				</c:when>
				<c:otherwise>
					...
					<span class="age-range">
						<c:choose>
							<c:when test="${entry.passengerType.code == 'child'}">
                                                            (<spring:theme
									code="text.cms.accommodationfinder.passengerType.label.up.to" text="up to " />&nbsp${fn:escapeXml(entry.passengerType.maxAge)}
                                                        </c:when>
							<c:otherwise>
